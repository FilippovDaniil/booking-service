...
Фронтенд — это статические HTML-файлы в папке `frontend/`. Откройте нужный файл прямо в браузере (двойной клик или перетащите в окно браузера):

| Файл | Для кого | Что умеет |
|------|----------|-----------|
| `frontend/index.html` | Все | Главная страница: вход и регистрация |
| `frontend/client.html` | `CLIENT` | Поиск квартир, создание броней, отзывы |
| `frontend/landlord.html` | `LANDLORD` | Управление своими квартирами и бронями |
| `frontend/admin.html` | `ADMIN` | Статистика и управление пользователями |

Начинайте с `index.html` — зарегистрируйтесь или войдите, после чего страница сама перенаправит в нужный кабинет.

> Фронтенд обращается к API по адресу `http://localhost:8555`, поэтому приложение должно быть запущено.
